...
// Standalone wrapper for BookingFeed when accessed via direct route
function BookingFeedPage() {
  const navigate = useNavigate()
  return <BookingFeed onBack={() => navigate('/home', { replace: true })} />
}
  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          !user ? <Navigate to="/login" replace /> : <BookingFeedPage />
        }
      />
      <Route
        path="/history"
        element={
          !user ? <Navigate to="/login" replace /> : (
            <div style={{ height: '100dvh', overflowY: 'auto', background: 'var(--bg-0)' }}>
              <HistoryScreen />
            </div>
          )
        }
